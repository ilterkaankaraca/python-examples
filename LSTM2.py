...
sc = StandardScaler() 
x = sc.fit_transform(x.values) # Standartize ediyor 
x_train, x_valid, y_train, y_valid = train_test_split(x, y, test_size=0.3) #veri setini ayırıyor
...
#%%
model = Sequential()
model.add(LSTM(2,input_shape = (nb_features,1)))
...
#%%
model.compile(optimizer='rmsprop', loss='categorical_crossentropy', metrics=['acc',f1_m,precision_m, recall_m, tf.keras.metrics.AUC()])

...
from tensorflow.keras.layers import Dense,Dropout,Embedding,BatchNormalization, Conv1D, GlobalAveragePooling1D, MaxPool1D
from tensorflow.keras.optimizers import Adam

ROOT_DIR = 'E:\\Repos\\python-examples\\dataset\\audio-cats-and-dogs\\cats_dogs\\'
CSV_PATH = 'E:\\Repos\\python-examples\\dataset\\audio-cats-and-dogs\\train_test_split.csv'
...
model.add(BatchNormalization())
model.add(Conv1D(80,4, strides=2, activation='softmax'))
model.add(BatchNormalization())
...
adam_optimizier = Adam(decay=1e-3)
model.compile(loss='binary_crossentropy',optimizer=adam_optimizier, metrics=['accuracy'])
...
for epochs in range(NUM_EPOCHS):
    ...
    for batch_x, batch_y in train_gen:
        ...
        valid_acc.extend(history.history['val_accuracy'])
...
fig= plt.figure(figsize=(15,8))
ax = fig.add_subplot(111)
ax.plot(train_loss, label="train accuracy")
